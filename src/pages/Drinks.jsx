...
function Drinks() {
    ...
    return (
        <main className="drinks-list-page" id="main-content" onClick={handleClosePopup}>
            ...
            <div className="drinks-list-container">
                {drinksItemsToDisplay.map(section => (
                    <section key={section.title} className="drinks-section">
                        <h2 className="drinks-section-title">{section.title}</h2>
                        <div className="drinks-section-items"
                        ...
                        >
                            {section.data.map(item => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className="drinks-item-title">{item.name}</div>
                                        <div className="drinks-item-price">{item.price}</div>
                                        {/* Popup */}
                                        {popupItem === item && (
                                            <div
                                                className={`drinks-item-popup${isMobile ? " mobile" : ""}`}
                                                id={popupId}
                                            ...
                                            >
                                                ...
                                                <img src={import.meta.env.BASE_URL + item.img}
                                                    alt={item.name}
                                                    className="drinks-popup-img"
                                                />
                                                <div className="drinks-popup-info">
                                                    <div className="drinks-popup-calories">{item.nutrition.calories} kcal </div>
                                                    <div className="drinks-popup-sugar">{item.nutrition.sugar} in grams </div>
                                                </div>
                                            </div>
                                        )}
                                    </div>
