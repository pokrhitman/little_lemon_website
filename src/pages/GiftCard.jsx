...
export default function GiftCard() {
    ...
    function handleSpend() {
        ...
        setHistory([
            ...history,
            { date: new Date().toLocaleString(), amount, code: giftCard.code },
        ]);
        ...
        <div className="giftcard-container">
            ...
            {!giftCard ? (
                <div className="giftcard-load">
                    ...
                    <button className="giftcard-btn" onClick={handleLoadCard} style={{ marginLeft: 8 }}>Check Balance</button>
                ...
                <div className="giftcard-details">
                    ...
                    />
                    <button className="giftcard-btn" onClick={handleSpend} style={{ marginLeft: 8 }}>Spend</button>
                    <button className="giftcard-btn secondary" onClick={handleReset} style={{ marginLeft: 8 }}>New Card</button>
                    {message && <p className="giftcard-message-success">{message}</p>}
                    <h4>Redemption History</h4>
                    <ul className="giftcard-history">
                        {history.map((h, i) => (
                            <li key={i}>{h.date}: Spent ${h.amount.toFixed(2)}</li>
                        ))}
                    </ul>
                </div>
            )}
